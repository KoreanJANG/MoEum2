...
'''220711 ver1.1 / chromedriver 경로 = aws ec2

              수정) 1. Title, Desc., Image의 data 탐색 (Ex. title의 경우, meta -> title -> h1 -> h2 tag 중 가장 적합한 data 선정) 
                     2. 특정 사이트 구조에 맞는 data 탐색 (Ex. 내부 api or java script를 활용한 json data)
                     3. 위 1에서 찾은 data를 저장* 후 위 2에서 데이터를 찾을 경우 덮어씌움 (*db가 아닌 파이썬 내 논리적 저장을 의미)
...
try:
    ...
    try:
        Title_key = soup.select_one('meta[property="twitter:title"]')['content']
    except:
        try:
            Title_key = soup.select_one('title').get_text()
        except:
            try:
                Title_key = soup.select_one('h1').get_text()
            except:
                try:
...
# 고도화 필요. 예외로 찾은 len.string과 비교하여 긴 것으로 타이틀 정함? 또는 반복되는 특정 단어가 포함된 string 중 가장 긴 것?

try:
    Description_key = soup.select_one('meta[property="og:description"]')['content']
except:
    try:
        Description_key = soup.select_one('meta[property="twitter:description"]')['content']
    except:
        try:
            Description_key = soup.select_one('strong').get_text()
        except:
            try:
                Description_key = soup.select_one('h1').get_text()
            except:
                try:
                    Description_key = soup.select_one('h2').get_text()
                except:
...
if Type_key == '쇼핑':
    ...
    try:
        ...
        if Distributor_key in ['11st']:
            ...
            Lower_price_key = soup.select_one('div.b_product_info_price.b_product_info_price_style2 > div > div > ul > li > dl.price > dd > strong > span.value').text
