...
print("User_url(Redirection 가능성 있음)은, ", User_url)
...
Category_in_keyword_list_shopping = ['11st', 'coupang', 'musinsa', 'a-bly', 'zigzag', 'brandi', 'gmarket', 'oliveyoung',
                                     ...
                                     'tmon', 'goods']
Category_in_keyword_list_blog = ['blog', 'tistory', 'velog', 'github']
Category_in_keyword_list_sns = ['instagram', 'band', 'facebook']
Category_in_keyword_list_video = ['youtube', 'tiktok']
Category_in_keyword_list_second = [
    'daangn', 'joonggonara', 'joongna', 'bunjang', 'products', 'shop']
Category_in_keyword_list_cafe = ['cafe']
Category_in_keyword_list_news = ['news', 'joongang', 'yna']
...
for Category_in_keyword_list_all_i in Category_in_keyword_list_all:
    Category_in_keyword_match_list = list(
        set(User_url_list).intersection(Category_in_keyword_list_all_i))
...
if max(Category_in_keyword_match_list_cnt_dict.keys()) == 0:
    ...
    Category_in_keyword_match_list_cnt_dict_keys_max_values = Category_in_keyword_match_list_cnt_dict[max(
        Category_in_keyword_match_list_cnt_dict.keys())]
...
#     res_noifr.status_code
    ...
    for product_id_bunjang in product_id_bunjang1:
        User_url_api = 'https://api.bunjang.co.kr/api/1/product/' + \
            str(product_id_bunjang) + '/detail_info.json?version=4'
...
# lprice&mall : Type = 쇼핑인 경우만 실행, try & except 로 모든 경우의 수 셋팅. else = 해당 링크에서 직접 보기(DJI 비교)
...
# 쇼핑 Distributor 주요 0개사 지정
    ...
    # 네이버쇼핑(searched, brand, smartstore 포함)
        ...
        try:  # Searched
            ...
            Lower_mall_key = soup.select_one(
                'span.lowestPrice_mall__2r0yz'), text
        except:
            try:
                Lower_price_key = soup.select_one(
                    'span._1LY7DqCnwR').text  # Smartstore, brand
            except:
                Lower_price_key = "해당 링크에서 직접 보기"
    ...
    # 에이블리
        ...
        for product_id_alby in product_id_alby1:
            User_url_api = 'https://api.a-bly.com/webview/goods/' + \
                str(product_id_alby)
    ...
    # 지그재그
        ...
        for product_id in product_id1:
            User_url_rd = 'https://store.zigzag.kr/catalog/products/' + \
                str(product_id)
...
#         print(box)
    ...
    if User_url in ['naver.com'] and Type == '쇼핑':
    ...
    try:
        Title_pre_key = re.sub(
            r'\[([가-힣0-9]*?)\]|\(([가-힣0-9]*?)\)|\|([가-힣0-9]*?)', '', Title_key)
    ...
    pattern2 = re.compile("[A-Za-z0-9\/\-]{8,10}")
#     pattern2_1 = re.compile("[A-Za-z\d/]+[A-Za-z][a-zA-Z\d]{2}[\d]+|[A-Za-z\d/]+[\d][a-zA-Z\d]{2}[A-Za-z][A-Za-z\d/]+")
    ...
    if Title_chosen_key == "해당 링크에서 직접 보기":
        ...
        searched_URL = 'https://search.shopping.naver.com/search/all?query=' + \
            str(Title_chosen_key)

        res_searched = requests.get(searched_URL, headers=headers)

        print(res_searched.status_code)

        soup_searched = BeautifulSoup(res_searched.content, 'html.parser')
        basiclist_searched = soup_searched.find(
            'li', class_='basicList_item__2XT81')  # ad 상품 제외한 일반 상품 태그

        # print(basiclist_searched.text)

    # for basiclist1_searched in basiclist_searched:
        Title_searched_key = basiclist_searched.select_one(
            'a.basicList_link__1MaTN')['title']
        Lower_price_searched_key = basiclist_searched.select_one(
            'span.price_num__2WUXn').get_text()

        try:
            Lower_mall_searched_key = basiclist_searched.select_one(
                'span.basicList_mall_name__1XaKA').get_text()
        except:
            try:
                Lower_mall_searched_key = basiclist_searched.select_one(
                    'a.basicList_mall__sbVax').get_text()
                if Lower_mall_searched_key == '':
                    # 최저가 쇼핑몰에 로고만 붙은 경우 alt값 추출
                    Lower_mall_searched_key = basiclist_searched.select_one(
                        'a.basicList_mall__sbVax > img').get('alt')
            except:
                Lower_mall_searched_key = "해당 링크에서 직접 보기"

        print("네이버쇼핑 최저가는, ", Lower_price_searched_key)

    Lower_url_searched_key = basiclist_searched.select_one(
        'a.basicList_link__1MaTN')['href']
    ...
    Lower_price_searched_key = re.sub(
        r'([^0-9]*?)', '', Lower_price_searched_key)
...
# sql = 'insert into posts values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)'
# sql = 'insert into posts values(%s, %s, %s, %s, %s, %s)'
# vals = (Type_key, Distributor_key, Thumbnail_image_key, User_url, Title_key, Lower_price_key)
